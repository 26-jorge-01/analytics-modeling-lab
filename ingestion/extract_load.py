...
def clean_table_name(filename):
    ...
    """
    name = filename.replace("olist_", "").replace(
        "_dataset", "").replace(".csv", "")
    return name
...
def load_csv_to_postgres():
    ...
    for file_path in csv_files:
        ...
        logger.info(
            f"Loading {file_path.name} into {RAW_SCHEMA}.{table_name}..."
        )

        try:
            # Load with pandas
            # Load with pandas
            # Note: Geolocation file is large, using low_memory=False
