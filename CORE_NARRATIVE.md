...
*   **The NaN Time Bomb**: JSON APIs often serialize missing dates as the literal string `'NaN'` rather than JSON `null`. In PostgreSQL, `cast('NaN' as timestamp)` throws a hard error. The fix—wrapping all raw-date casts with `nullif(cast(col as text), 'NaN')`—prevented the entire pipeline from crashing on 5–10% of records that had administrative metadata gaps. This is a recurring pattern in public-sector APIs where legacy systems export "empty" dates as NaN.
*   **Key Generation Mismatch in ER**: The surrogate key for agencies was generated from `(canonical_nit, subdivision_type, departamento)` in the golden dimension, but the contracts fact table generated its foreign key from `nit_entidad` alone. This meant every single contract failed referential integrity (5.6M FK violations). The fix—joining the fact table with the resolved dimension to acquire the correct key—highlights a fundamental ER design rule: **foreign keys must always be sourced from the dimension, never independently derived**.
*   **QUALIFY Is Not Portable**: Snowflake's `QUALIFY` clause is convenient but not standard SQL. When porting to PostgreSQL, it must be rewritten as a subquery with `row_number() + WHERE rn = 1`. Testing model portability early prevents last-minute rewrites during deployment.
*   **Severity Triage for Legacy Gaps**: 2% of contracts (113k records) lack both `departamento` and `municipio_de_obtencion`. This is a genuine business anomaly (missing geographic metadata on old contracts), but it should not block 98% of the pipeline value. Downgrading the `not_null` test to `warn` applied the "Severity Triage" pattern: halt for corruption, flag for business gaps.
