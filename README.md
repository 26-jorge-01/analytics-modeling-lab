...
| **Core 3NF (Silver)** | [core/README.md](./transform/dbt/models/intermediate/core/README.md) | Enriched referential integrity, identity resolution |
...
| **❄️ Snowflake Model** | [snowflake/README.md](./transform/dbt/models/snowflake/README.md) | Normalized hierarchies, atomic updates |
...
- **Zero-Loss Resilience**: 11GB schema migration to canonical alphabetical structure (pysecop 1.4.4).
- **Audit-Grade Identity**: Synthetic fingerprinting and name consolidation ranking for Agencies and Vendors.
- **Enriched 3NF Layer**: Dimensions enriched with SME flags, Sector, Rama, and high-precision financial metrics.
- **Modern DX**: Hardened Medallion architecture using dbt, Dagster, and Soda.io.
...
The SECOP II dataset provides a realistic transactional stream: 5.6M+ contracts, complex procurement lifecycles, and multi-dimensional entity behavior. The complexity is intentional — clean data requires no architectural decisions.
...
# 3. Run the pipeline
.\make.bat ingest       # Extract & Load from SECOP API → Raw schema
.\make.bat dbt-build    # Transform: Raw → Staging → Core 3NF → Marts
